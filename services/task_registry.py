...
def register_task(user_id: int, task) -> None:
    """Регистрирует задачу пользователя. Если уже есть активная — отменяет старую,
    чтобы не плодить параллельные обработки одного и того же пользователя."""
    existing = _active_tasks.get(user_id)
    if existing and not existing.done():
        existing.cancel()
        logger.warning(f"🔁 Отменена предыдущая активная задача: user_id={user_id}")
