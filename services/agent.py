...
async def run_agent(user_id: int, message: str) -> str:
    ...
    system_prompt = (
        ...
        # === ЖЁСТКОЕ ПРАВИЛО ===
    "КРИТИЧЕСКОЕ ПРАВИЛО: Ты ДОЛЖЕН использовать ReAct-формат ВСЕГДА, "
    "даже если думаешь, что знаешь ответ. Никаких прямых ответов пользователю "
    "без предварительного рассуждения в формате ReAct.\n\n"
    
    "Порядок работы:\n"
    "1. Plan: составь план решения\n"
    "2. Thought: подумай, какой инструмент нужен (или не нужен)\n"
    "3. Action: имя инструмента ИЛИ 'final_answer' если инструменты не нужны\n"
    "4. Action Input: JSON с параметрами (или {} для final_answer)\n"
    "5. Predict: что ожидаешь получить\n\n"
    
    "Если инструменты НЕ нужны:\n"
    "Thought: [рассуждение]\n"
    "Action: final_answer\n"
    "Action Input: {}\n"
    "Predict: [что ожидаешь]\n\n"
    
    "После Observation:\n"
    "Plan Update: [статус]\n"
    "(повтори цикл)\n\n"
    
    "ФИНАЛЬНЫЙ ОТВЕТ ТОЛЬКО через Action: final_answer + Final Answer: [текст]\n"
    "ЗАПРЕЩЕНО: отвечать пользователю без Action: final_answer.\n"
