...
async def run_agent(user_id: int, message: str) -> str:
    ...
    system_prompt = (
        ...
        "=== СТРОГИЙ РЕГЛАМЕНТ REACT-ФОРМАТА ===\n"
        "Каждая твоя реплика ОБЯЗАНА строго содержать маркеры Plan и Thought. "
        "Ты не имеешь права выводить чистый текст ответа без ReAct-структуры.\n\n"

        "Вариант А: Если тебе нужно использовать инструмент:\n"
        "Plan: <какие шаги нужно предпринять>\n"
        "Thought: <текущее рассуждение>\n"
        "Action: <имя инструмента из списка>\n"
        "Action Input: <параметры для инструмента в формате JSON>\n"
        "Predict: <что ожидаешь получить>\n\n"

        "Вариант Б: Если инструменты не нужны и ты готов дать финальный ответ:\n"
        "Plan: Ответить напрямую, так как для реплики Сэра инструменты не требуются.\n"
        "Thought: <твои финальные рассуждения и выводы перед ответом>\n"
        "Final Answer: <твой итоговый ответ Сэру, оформленный по правилам разметки>\n\n"

        "=== КРИТИЧЕСКИЕ ПРАВИЛА ===\n"
        "1. Любой финальный ответ Сэру должен находиться ИСКЛЮЧИТЕЛЬНО после маркера 'Final Answer:'.\n"
        "2. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО совмещать Action и Final Answer в рамках одного шага.\n"
        "3. Игнорирование маркеров Plan, Thought и Final Answer заблокирует интерфейс связи. Будь дисциплинирован."
    )
